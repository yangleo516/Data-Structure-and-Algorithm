...
//	long[] timeRecorder = new long[1500];      //Record 100 times
...
//		points = genPointsAtRandom(nPoints, prng);
	    ...
	    DCSum = DCSum + elapsedTime;
//	    timeRecorder[i] = elapsedTime;			//Get running time
...
//	long max = timeRecorder[0];
//	long min = timeRecorder[0];
//	long sumTime = 0;
//	long sumVariance = 0;
//	long variance = 0;
	long dcAverage = 0;
//	for(int i=0;i<1500;i++){
/*		if(timeRecorder[i] > max){
			max = timeRecorder[i];
		}
		if(timeRecorder[i] < min){
			min = timeRecorder[i];
		} */
//		sumTime = sumTime + timeRecorder[i];
//	}
	dcAverage = DCSum / N;
/*	for(int i=0;i<100;i++){
		sumVariance = sumVariance + (long)Math.pow(timeRecorder[i]-dcAverage, 2);
	}
	variance = sumVariance / 100;
	System.out.println("The maxmum time is     " + max);
	System.out.println("The minimum time is    " + min); */
	System.out.println("The DC sum is          " + DCSum);
	System.out.println("The DC average time is " + dcAverage);
...
/*	    System.out.println("For n = " + points.length + 
	    ...
	    sumNaive = sumNaive + elapsedTime;
	}
	aveNaive = sumNaive / N;
	System.out.println("The Naive sum is          " + sumNaive);
	System.out.println("The Naive average time is " + aveNaive);
    }
