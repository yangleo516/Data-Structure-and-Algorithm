...
{
  // priority queue test
  if (argc == 3 && !strcmp(argv[1], "pqtest"))
    {
      cout << "pqtest\n";
      
      PQTest(strtoul(argv[2], nullptr, 10));
      return 0;
    }
  
  if (argc < 4)
    {
      cerr << "Syntax: Main <airports> <flights> <queries>\n";
      return -1;
    }
...
static void findPaths(Input &input, Multigraph &G, const char *queryFile)
{
  ...
  for (unsigned int i = 0; i < queries->size(); i++)
	...
	(it == input.airportMap.end() ? nullptr : it->second);
